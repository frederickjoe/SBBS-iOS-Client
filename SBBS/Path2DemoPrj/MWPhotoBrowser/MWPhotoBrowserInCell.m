...
// Private
@interface MWPhotoBrowserIncell () {
    
	// Data
    id <MWPhotoBrowserIncellDelegate> _delegate;
...
// MWPhotoBrowser
@implementation MWPhotoBrowserIncell
        ...
        if ([self numberOfPhotos] > 0 && [photo underlyingImage]) {
            ...
            if ([MFMailComposeViewController canSendMail]) {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        ...
                                                        otherButtonTitles:NSLocalizedString(@"保存", nil), NSLocalizedString(@"复制", nil), NSLocalizedString(@"邮件分享", nil), nil] autorelease];
            } else {
                self.actionsSheet = [[[UIActionSheet alloc] initWithTitle:nil delegate:self
                                                        cancelButtonTitle:NSLocalizedString(@"取消", nil) destructiveButtonTitle:nil
...
- (void)actuallyCopyPhoto:(id<MWPhoto>)photo {
    if ([photo underlyingImage]) {
        ...
        [self showProgressHUDCompleteMessage:NSLocalizedString(@"已复制", @"Informing the user an item has finished copying")];
        [self hideControlsAfterDelay]; // Continue as normal...
    }
}
...
- (void)mailComposeController:(MFMailComposeViewController*)controller didFinishWithResult:(MFMailComposeResult)result error:(NSError*)error {
    if (result == MFMailComposeResultFailed) {
		UIAlertView *alert = [[[UIAlertView alloc] initWithTitle:NSLocalizedString(@"邮件分享", nil)
                                                         message:NSLocalizedString(@"邮件发送失败.", nil)
                                                        delegate:nil cancelButtonTitle:NSLocalizedString(@"好", nil) otherButtonTitles:nil] autorelease];
		[alert show];
    }
	[self dismissModalViewControllerAnimated:YES];
}
