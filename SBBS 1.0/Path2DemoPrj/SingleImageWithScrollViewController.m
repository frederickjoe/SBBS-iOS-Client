...
{// Was there an error?
    if(error !=NULL){
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"警告" message:@"图片保存失败" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil];
        [alert show];
        [alert release];
    }
    else
    {
        UIAlertView *alert = [[UIAlertView alloc] initWithTitle:@"提示" message:@"图片保存成功" delegate:self cancelButtonTitle:@"ok" otherButtonTitles:nil];
        [alert show];
        [alert release];
    }
}
