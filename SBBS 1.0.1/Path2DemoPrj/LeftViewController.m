...
- (void)viewDidLoad {
    ...
    {
        tableTitles2 = [NSArray arrayWithObjects:@"收藏",@"好友",@"站内信", @"新消息", @"登出",nil];
...
-(void)LoginSuccess
{
    NSArray * new = [NSArray arrayWithObjects:@"收藏",@"好友",@"站内信", @"新消息",@"登出",nil];
    if (tableTitles2 != new) {
        [tableTitles2 release];
        tableTitles2 = new;
        [tableTitles2 retain];
...
- (void)actionSheet:(UIActionSheet*)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    if(buttonIndex == 0)
    {
        ...
        NSArray * new2 = [[NSArray arrayWithObjects:@"未登录", nil] retain];
        if (tableTitles3 != new2) {
            [tableTitles3 release];
            tableTitles3 = new2;
            [tableTitles3 retain];
